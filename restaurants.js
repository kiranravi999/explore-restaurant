...
const getRestaurants=async()=>{
   ...
   try{
    ...
    error.style.display='block';
      error.textContent=`Error: Failed to Load ${err.message}`
   }
...
const onDelete= async(id)=>{
    ...
    try{
        ...
        if (!response.ok){
            throw new Error (`Failed to delete the data:${response.status}`);
        }
        getRestaurants();
        ...
        loading.style.display='none';
        error.style.display='block';
          error.textContent=`Error: Failed to Delete ${err.message}`
       }
